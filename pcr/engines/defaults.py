...
from pcr.utils.misc import intersection_and_union_gpu, shape_intersection_and_union_gpu, intersection_and_union
...
class Trainer:
    ...
    def run_step(self, i, input_dict):
        ...
        if (i+1) % 100 == 0:
            if self.cfg.empty_cache:# and (i+1) % 10 == 0:
                torch.cuda.empty_cache()
    ...
    def after_epoch(self):
        ...
        if self.cfg.data.ignore_label == 0:
    ...
    def save_checkpoint(self):
        if comm.is_main_process():
            ...
            if is_best:
                ...
                if self.epoch + 1 > 25:
                    shutil.copyfile(filename, os.path.join(self.cfg.save_path, 'model', f'model_best_{self.epoch + 1}.pth'))
        ...
        if self.cfg.weight and os.path.isfile(self.cfg.weight):
            ...
            for k, v in self.model.state_dict().items():
                if k not in state_dict:
                    print(k, v.shape)
            load_state_info = self.model.load_state_dict(state_dict, strict=False)
