...
class ToTensor(object):
    def __call__(self, data):
        if isinstance(data, Mapping):
            ...
            #         print(sub_key, type(item), item.dtype, np.issubdtype(item.dtype, np.int), np.issubdtype(item.dtype, np.floating))
...
class NormalizeColor(object):
    def __call__(self, data_dict):
        if "color" in data_dict.keys():
            ...
            # data_dict["color"] = data_dict["color"] / 255 - 0.5
        return data_dict


@TRANSFORMS.register_module()
class NormalizeColor01(object):
    def __call__(self, data_dict):
        if "color" in data_dict.keys():
            data_dict["color"] = np.clip(data_dict["color"] / 255, 0, 1)
        return data_dict
...
@TRANSFORMS.register_module()
class RandomGaussianShift(object):
...
class RandomNormalDrop(object):
    ...
    def __call__(self, data_dict):
        if "normal" in data_dict.keys() and np.random.rand() < self.p:
            data_dict["normal"] *= self.color_augment
        return data_dict
...
class ElasticDistortion(object):
    def __init__(self, distortion_params=None, application_ratio=0.95):
...
class Voxelize(object):
    def __init__(self,
        ...
        self.average_feat = average_feat
    ...
    def __call__(self, data_dict):
        ...
        if self.mode == 'train':  # train mode
        ...
        elif self.mode == 'val':  # val mode
            idx_select = np.cumsum(np.insert(count, 0, 0)[0:-1])
            idx_unique = idx_sort[idx_select]
            data_dict["index"] = np.arange(len(discrete_coord))
            if self.return_discrete_coord:
                data_dict["discrete_coord"] = discrete_coord[idx_unique]
            if self.return_inverse:
                data_dict["mask"] = np.zeros_like(inverse)
            ...
            for key in self.keys:
                if key != "label":
                    if self.average_feat:
                        feat_tensor = torch.FloatTensor(data_dict[key])
                        idx_recon = torch.LongTensor(data_dict["inverse"])
                        data_dict[key] = scatter_mean(feat_tensor, idx_recon, dim=0).numpy()
                    else:
                        data_dict[key] = data_dict[key][idx_unique]
            return data_dict
            ...
            for i in range(count.max()):
                ...
                if self.return_inverse:
                    ...
                    data_part["index"] = np.arange(len(discrete_coord))
                if self.return_min_coord:
                    data_part["min_coord"] = min_coord.reshape([1, 3])
                if "cls_token" in data_dict:
                    data_part["cls_token"] = data_dict["cls_token"]
...
## Fast Point Transformer Augmentation
@TRANSFORMS.register_module()
class DimensionlessCoordinates(object):
    ...
    def __call__(self, data_dict):
        if "coord" in data_dict.keys():
            data_dict["coord"] = data_dict["coord"] / self.voxel_size
        return data_dict
...
class RandomCrop(object):
    ...
    def __call__(self, data_dict):
        if "coord" not in data_dict.keys() or random.random() > self.application_ratio :
            return data_dict

        norm_coords = data_dict["coord"] - data_dict["coord"].min(0, keepdims=True)
        ...
        if valid:
            data_dict["coord"] = data_dict["coord"][sel]
            if "color" in data_dict.keys():
                data_dict["color"] = data_dict["color"][sel]
            if "normal" in data_dict.keys():
                data_dict["normal"] = data_dict["normal"][sel]
            if "label" in data_dict.keys():
                data_dict["label"] = data_dict["label"][sel]
        return data_dict
...
class RandomRotation(object):
    ...
    def __call__(self, data_dict):
        if random.random() < self.application_ratio:
            ...
            data_dict["coord"] = data_dict["coord"] @ M(axis, angle)
        return data_dict
...
class RandomTranslation(object):
    ...
    def __call__(self, data_dict):
        if random.random() < self.application_ratio:
            data_dict["coord"] += 2 * (np.random.rand(1, 3) - 0.5) * self.max_translation
        return data_dict

@TRANSFORMS.register_module()
class NormalizeColorFPT(object):
    def __call__(self, data_dict):
        if "color" in data_dict.keys():
            data_dict["color"] = data_dict["color"] / 256 - 0.5
        return data_dict
