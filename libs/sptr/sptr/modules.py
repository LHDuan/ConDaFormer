...
def sparse_self_attention(query, 
    ...
    if pe_type == 'contextual' and rel_query and rel_key:
        ...
        # assert (relative_position_index<2*quant_grid_length-1).all()
        # if xyz.device == torch.device('cuda:0'):
        #     print(torch.max(relative_position_index[:,0]), torch.min(relative_position_index[:,0]))
        #     print(torch.max(relative_position_index[:,1]), torch.min(relative_position_index[:,1]))
        #     print(torch.max(relative_position_index[:,2]), torch.min(relative_position_index[:,2]))
        if split_func:
            relative_position_index = split_func(xyz_ctg, index_0, index_1, relative_position_index.clone())
            relative_position_index = torch.clamp(relative_position_index, 0, 2*quant_grid_length-2)
        
        relative_position_index = relative_position_index.int()
        # print(torch.max(relative_position_index), torch.min(relative_position_index), 2*quant_grid_length-1)
