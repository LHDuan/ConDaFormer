...
void attention_step1_tri_plane_backward_cuda(int N, int M, int h, int hdim, const unsigned int n_max, at::Tensor grad_out_tensor, 
    at::Tensor index0_tensor, at::Tensor index0_tensor_offsets, at::Tensor index1_tensor, at::Tensor index1_tensor_offsets, at::Tensor m_tensor_offsets, 
    ...
    at::Tensor grad_q_tensor, at::Tensor grad_k_tensor)
{
    ...
    const int *index1 = index1_tensor.data_ptr<int>();
    const int *index1_offsets = index1_tensor_offsets.data_ptr<int>();
    const int *m_offsets = m_tensor_offsets.data_ptr<int>();
    const float *q = q_tensor.data_ptr<float>();
    const float *k = k_tensor.data_ptr<float>();
    float *grad_q = grad_q_tensor.data_ptr<float>();
    float *grad_k = grad_k_tensor.data_ptr<float>();
    attention_step1_tri_plane_backward_cuda_launcher(N, M, h, hdim, n_max, grad_out, index0, index0_offsets, index1, index1_offsets, m_offsets, q, k, grad_q, grad_k);
}
