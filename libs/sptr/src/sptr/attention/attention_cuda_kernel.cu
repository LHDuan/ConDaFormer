...
__global__ void attention_step1_tri_plane_backward_cuda_kernel( // M, h, C//h
    int N, int M, int h, int d, const float *grad_out, const int *index0, const int *index0_offsets, const int *index1, const int *index1_offsets, const int *m_offsets, 
    const float *q, const float *k, float *grad_q, float *grad_k) {
    // q: [N, h, d], k: [N, h, d], index0: [M], index1: [M], attn: [M, h], grad_out: [M, h]
    // grad_q: [N, h, hdim], grad_k: [N, h, hdim]
...
void attention_step1_tri_plane_backward_cuda_launcher(int N, int M, int h, int hdim, const unsigned int n_max, 
    const float *grad_out, const int *index0, const int *index0_offsets, const int *index1, const int *index1_offsets, const int *m_offsets, 
    const float *q, const float *k, float *grad_q, float *grad_k) {  
    // input: grad_output: (n, nsample, c), output: grad_input1: (n, c), grad_input2: (n, c)
    ...
    attention_step1_tri_plane_backward_cuda_kernel<<<blocks, threads, 0>>>(N, M, h, hdim, grad_out, index0, index0_offsets, index1, index1_offsets, m_offsets, q, k, grad_q, grad_k);
