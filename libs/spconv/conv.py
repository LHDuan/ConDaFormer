...
class SparseConvolutionBase:
    def __init__(self,
        ...
        if groups>1:
            ...
            assert groups == in_channels, "channel wise groups only support groups = in_channels for now"
            assert out_channels == in_channels, "channel wise groups only support out_channels = in_channels for now"
            self.group_mask = torch.eye(in_channels).unsqueeze(0).expand(kv, -1, -1).float().cuda()
        if CPU_ONLY_BUILD:
        ...
        if self.algo == ConvAlgo.Native and not ALL_WEIGHT_IS_KRSC:
            if groups > 1:
                self.group_mask = self.group_mask.view(*self.kernel_size, in_channels, out_channels)
            if FILTER_HWIO:
                # RSCK
                weight_shape = [*self.kernel_size, in_channels//groups, out_channels]
            else:
                # RSKC
                weight_shape = [*self.kernel_size, out_channels, in_channels//groups]
        else:
            # KRSC
            weight_shape = [out_channels, *self.kernel_size, in_channels//groups]
            if groups > 1:
                self.group_mask = self.group_mask.permute(1,0,2).view(out_channels, *self.kernel_size, in_channels)
    ...
    def _conv_forward(self, training: bool, input: SparseConvTensor, weight: torch.Tensor, bias: Optional[torch.Tensor], add_input: Optional[SparseConvTensor] = None,
        ...
        if self.groups > 1 and self.groups == self.in_channels:
            # print(weight.shape, self.group_mask.shape)
            weight = weight * self.group_mask
